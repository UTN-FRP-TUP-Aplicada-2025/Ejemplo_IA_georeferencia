...
if not exist "src\GeoFoto.Tests\GeoFoto.Tests.csproj" (
    echo   [ERROR] No se encontro src\GeoFoto.Tests\GeoFoto.Tests.csproj
...
dotnet restore src\GeoFoto.Tests\GeoFoto.Tests.csproj --verbosity quiet
if errorlevel 1 ( echo   [ERROR] Restore fallo & pause & exit /b 1 )

:: Compilar
echo   [INFO] Compilando proyecto de tests...
dotnet build src\GeoFoto.Tests\GeoFoto.Tests.csproj --configuration Debug --verbosity quiet --no-restore
...
dotnet test src\GeoFoto.Tests\GeoFoto.Tests.csproj ^
    --configuration Debug ^
    --no-build ^
    --verbosity normal ^
