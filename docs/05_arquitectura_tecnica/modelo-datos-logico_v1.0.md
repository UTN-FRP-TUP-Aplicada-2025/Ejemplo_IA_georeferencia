...
| RadioMetros | `float` | NO | `50` | Radio de agrupación del marker en metros (default 50, rango 10–500). Configurable por el usuario. |
| IsDeleted | `bit` | NO | `0` | Soft-delete para sincronización: `1` indica eliminado. El registro permanece en BD hasta que todos los clientes confirmen la eliminación. |
...
| `IX_Puntos_IsDeleted` | `IsDeleted` | Filtrar registros activos eficientemente. |
...
| Comentario | `nvarchar(500)` | SÍ | `NULL` | Descripción individual de la foto, editable desde el carrusel o el visor fullscreen. |
| IsDeleted | `bit` | NO | `0` | Soft-delete para sincronización: `1` indica foto eliminada. |
...
| `IX_Fotos_IsDeleted` | `IsDeleted` | Filtrar fotos activas eficientemente. |
...
| RadioMetros | `REAL` | NO | `50` | Radio de agrupación del marker en metros (default 50, rango 10–500). |
| IsDeleted | `INTEGER` | NO | `0` | Soft-delete: `1` si el punto fue eliminado localmente, pendiente de confirmar en servidor. |
...
| Comentario | `TEXT` | SÍ | `NULL` | Descripción individual de la foto, editable desde el carrusel. |
| IsDeleted | `INTEGER` | NO | `0` | Soft-delete: `1` si la foto fue eliminada localmente, pendiente de confirmar en servidor. |
| UpdatedAt | `TEXT` | NO | | Última modificación en formato ISO 8601 UTC. |
| SyncStatus | `TEXT` | NO | `'Pending'` | Estado de sincronización: `Pending`, `Synced`, `Modified`, `Deleted`. |
...
| 1.1 | 2026-04-16 | Equipo Técnico | Sprint 07/08 — Agregados campos: `RadioMetros` e `IsDeleted` en `Puntos`/`Puntos_Local`; `Comentario` e `IsDeleted` en `Fotos`/`Fotos_Local`. Actualizados modelos C# sqlite-net-pcl y configuración EF Core. |
