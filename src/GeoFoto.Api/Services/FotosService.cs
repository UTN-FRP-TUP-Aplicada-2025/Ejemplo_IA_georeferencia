...
public class FotosService : IFotosService
{
    ...
    public async Task<FotoDto> UploadToPuntoAsync(int puntoId, IFormFile file, CancellationToken ct = default)
    {
        using var stream = file.OpenReadStream();
        var geo = _exif.ExtractGeoData(stream);
        ...
        return new FotoDto(foto.Id, foto.PuntoId, foto.NombreArchivo,
            foto.FechaTomada, foto.TamanoBytes, foto.LatitudExif, foto.LongitudExif);
    }
